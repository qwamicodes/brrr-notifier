...
export function createDokployDedupeKey(input: CreateDokployDedupeKeyInput): string {
  ...
  const environment = input.environment && input.environment.trim().length > 0 ? input.environment : 'default';
  const sourceEvent = input.source_event && input.source_event.trim().length > 0 ? input.source_event : 'unknown';
  const status = input.status ?? 'unknown';
  const occurredAt = input.occurred_at && input.occurred_at.trim().length > 0 ? input.occurred_at : 'unknown';
